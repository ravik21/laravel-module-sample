...
<div>
    <el-container>
        <el-header>
            <h1>{{ trans('users.title.traders') }}</h1>
            <el-breadcrumb separator="/">
                <el-breadcrumb-item :to="{name: 'admin.dashboard.index'}">{{ trans('core.breadcrumb.home') }}
                </el-breadcrumb-item>
                <el-breadcrumb-item :to="{name: 'admin.user.traders.index'}">{{ trans('users.breadcrumb.traders') }}
                </el-breadcrumb-item>
            </el-breadcrumb>
        </el-header>
        <el-main>
            ...
            <el-table
                ...
                <el-table-column prop="status" :label="trans('users.table.status')" width="200" class="text-center">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.status.class" close-transition>
                            {{ scope.row.status.name }}
                        </el-tag>
                        <el-tag v-if="scope.row.is_urgent && scope.row.pending_approval" type="danger" close-transition>
                            {{ trans('users.table.urgent') }}
                        </el-tag>
                    </template>
                </el-table-column>
                    ...
                    <template slot-scope="scope">
                        <el-dropdown split-button type="primary">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                ...
                                <view-table-item :to="{name: 'admin.user.traders.view', params: {id: scope.row.id}}">
                                </view-table-item>
                                <suspend-table-item :scope="scope" :rows="data" v-if="!scope.row.pending_approval">
                                </suspend-table-item>
                                <delete-table-item :scope="scope" :rows="data" v-if="permissions['user.traders.destroy']">
                                </delete-table-item>
                            </el-dropdown-menu>
                        </el-dropdown>
                    </template>
    ...
    export default {
        ...
        methods: {
            ...
            fetchUsers(customProperties) {
                ...
                axios.get(route('api.user.admin.traders.index', _.merge(properties, customProperties)))
                    .then((response) => {
            ...
            goToShow(scope) {
                this.$router.push({ name: 'admin.user.traders.view', params: { id: scope.row.id } });
            },
        },
        mounted() {
            this.fetchUsers();
